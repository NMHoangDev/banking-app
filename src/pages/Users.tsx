...
const initialAppUsers = [
  {
    ...
    dept: "IT Security",
    status: "Active",
    email: "thanh.lv@bank.vn",
    phone: "0901234567",
    lastAccess: "12/10/2023 14:22",
  },
  {
    id: "USR-002",
    ...
    dept: "Customer Care",
    status: "Active",
    email: "phuong.nt@bank.vn",
    phone: "0901234568",
    lastAccess: "12/10/2023 13:45",
  },
  {
    id: "USR-003",
    ...
    status: "Inactive",
    email: "hoang.tm@bank.vn",
    phone: "0901234569",
    lastAccess: "10/10/2023 09:12",
  },
  {
    id: "USR-004",
    name: "Vũ Anh Duy",
    ...
    dept: "Hanoi Branch",
    status: "Active",
    email: "duy.va@bank.vn",
    phone: "0901234570",
    lastAccess: "12/10/2023 15:50",
  },
  {
    id: "USR-005",
    name: "Phạm Minh Tuấn",
    ...
    status: "Suspended",
    email: "tuan.pm@bank.vn",
    phone: "0901234571",
    lastAccess: "01/10/2023 17:30",
  },
];
...
const initialRoles = [
  {
    name: "Administrator",
    desc: "Toàn quyền quản trị hệ thống, phê duyệt cấu hình bảo mật",
    userCount: 3,
  },
  {
    name: "Support Specialist",
    desc: "Hỗ trợ khách hàng, tra cứu thông tin cơ bản",
    userCount: 12,
  },
  {
    name: "Risk Auditor",
    desc: "Kiểm toán hệ thống, theo dõi và lập báo cáo rủi ro",
    userCount: 5,
  },
  {
    name: "Branch Manager",
    desc: "Quản lý chi nhánh, phê duyệt khoản vay hạn mức vừa",
    userCount: 8,
  },
  {
    name: "Developer",
    desc: "Phát triển, bảo trì và tích hợp hệ thống số",
    userCount: 15,
  },
];
...
const initialPermissions = [
  {
    code: "VIEW_TXN",
    name: "Xem giao dịch",
    desc: "Quyền xem danh sách và chi tiết các giao dịch",
  },
  {
    code: "CREATE_TXN",
    name: "Tạo giao dịch",
    desc: "Quyền thực hiện giao dịch chuyển tiền/thanh toán",
  },
  {
    code: "APPROVE_LOAN",
    name: "Phê duyệt khoản vay",
    desc: "Quyền xét duyệt hồ sơ tín dụng",
  },
  {
    code: "ISSUE_CARD",
    name: "Phát hành thẻ",
    desc: "Quyền duyệt cấp mới và khóa thẻ",
  },
  {
    code: "MANAGE_USERS",
    name: "Quản lý người dùng",
    desc: "Quyền thêm/sửa/xóa tài khoản nội bộ",
  },
  {
    code: "MANAGE_ROLES",
    name: "Quản lý vai trò",
    desc: "Quyền gán phân quyền cho các nhóm",
  },
];
...
const initialAuditLogs = [
  {
    ...
    action: "Reset password for USR-005",
    status: "SUCCESS",
  ...
  },
  {
    id: "LOG-002",
    time: "13:58:45",
    operator: "hoang.tm@bank.vn",
    action: "Exported Risk Assessment Report",
    status: "SUCCESS",
    target: "Risk Auditor",
  },
  {
    id: "LOG-003",
    time: "13:15:22",
    operator: "phuong.nt@bank.vn",
    action: "Issued Card to 1000000003",
    status: "SUCCESS",
    target: "Cards",
  },
  {
    id: "LOG-004",
    time: "11:42:00",
    operator: "duy.va@bank.vn",
    action: "Approved Loan L-998271",
    status: "SUCCESS",
    target: "Loans",
  },
];
...
export default function UsersPage() {
  const [activeTab, setActiveTab] = useState<
    "USERS" | "ROLES" | "PERMISSIONS" | "MATRIX" | "AUDIT"
  >("USERS");
  ...
  const handleToggleUserStatus = (userId: string) => {
    ...
    updateUser(apiId, { status: nextStatus })
      .then(() => {
        const log = {
      ...
      .catch((err) => {
        ...
        setUsers(prevUsers);
        if (selectedUser && selectedUser.id === userId) setSelectedUser(user);
      });
  };
  ...
  const handleResetPassword = (user: any) => {
    ...
    setAuditLogs((prevLogs) => [log, ...prevLogs]);
    alert(
      `Đã reset mật khẩu tài khoản ${user.name} thành công. Mật khẩu mới được gửi về email ${user.email}`,
    );
  };
    ...
    const load = async () => {
      ...
      try {
        ...
        const mapped = rows.map((u: any) => ({
          apiId: u.user_id,
          id: `USR-${String(u.user_id).padStart(3, "0")}`,
          name: u.username,
          role: u.role_name || "User",
          dept: u.customer_id ? `Customer ${u.customer_id}` : "N/A",
        ...
        setUsersError(
          "Không thể tải danh sách người dùng, hiển thị dữ liệu mẫu.",
        );
        ...
        <div>
          <nav className="flex text-[11px] font-bold text-on-surface-variant mb-2 gap-2 uppercase tracking-widest items-center">
            ...
            <span className="text-primary">
              Kiểm soát truy cập & Administration
            </span>
          </nav>
                ...
                <tbody>
                  {filteredUsers.map((user) => (
                    <tr key={user.id}>
                      ...
                      <td className="text-center">
                        <span className="text-xs font-bold text-[#002147] bg-primary-fixed/20 px-2.5 py-1 rounded-md">
                          {user.role}
                        </span>
                      </td>
                      <td className="text-center">
                        <p className="text-[10px] font-mono font-bold text-on-surface-variant">
                          {user.email}
                        </p>
      ...
      {activeTab === "ROLES" && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {roles.map((role) => (
            <div
              key={role.name}
              className="bg-white border border-outline-variant rounded-3xl p-6 shadow-sm hover:-translate-y-1 hover:shadow-md transition-all flex flex-col justify-between"
            >
              <div>
              ...
              <div className="pt-4 mt-6 border-t border-outline-variant/30 flex justify-between items-center text-xs">
                ...
                <span className="font-bold text-emerald-600">
                  {(rolePermissions[role.name] || []).length} Quyền hạn
                </span>
              </div>
            </div>
          ))}
        ...
        <div className="bg-white border border-outline-variant rounded-2xl shadow-sm overflow-hidden flex flex-col">
          <div className="p-4 bg-surface-container-low/50 border-b border-outline-variant">
            <h4 className="text-sm font-bold text-primary">
              Bảng ma trận gán quyền bảo mật dựa trên vai trò (RBAC)
            </h4>
            <p className="text-xs text-on-surface-variant mt-1">
              Ấn vào các checkbox bên dưới để gán hoặc hủy quyền trực tiếp cho
              từng Nhóm Vai Trò trong hệ thống
            </p>
          </div>
              ...
              <thead>
                <tr className="bg-surface-container">
                  ...
                  {permissions.map((p) => (
                    <th
                      key={p.code}
                      className="text-center px-4 py-4 max-w-[120px] whitespace-normal"
                      title={p.desc}
                    >
                      <div className="text-[10px] font-bold text-[#002147] leading-tight">
                        {p.name}
                      </div>
                      <div className="text-[8px] font-mono text-outline mt-0.5">
                        {p.code}
                      </div>
                    </th>
                  ))}
                      ...
                      return (
                        <td key={p.code} className="text-center px-4 py-4">
                          <input
                            ...
                            checked={isAssigned}
                            onChange={() =>
                              handleTogglePermissionInMatrix(role.name, p.code)
                            }
        ...
        <div className="bg-white border border-outline-variant rounded-2xl shadow-sm overflow-hidden">
          <table className="data-table">
            ...
            <tbody>
              {auditLogs.map((log) => (
                <tr key={log.id}>
                  <td className="font-mono text-xs font-bold text-[#002147]">
                    {log.id}
                  </td>
                  ...
                  <td>
                    <span className="text-xs font-bold">{log.operator}</span>
                  </td>
                  <td>
