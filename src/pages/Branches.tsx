...
const initialBranches = [
  { id: '1', name: 'Trụ sở chính - Enterprise Tower', type: 'Chi nhánh/PGD', address: 'Số 1 Đào Duy Anh, Đống Đa, Hà Nội', phone: '024 3998 1122', hours: '08:00 - 17:00', status: 'Open', region: 'Miền Bắc' },
  { id: '2', name: 'Chi nhánh Sài Gòn', type: 'Chi nhánh/PGD', address: '99 Lê Thánh Tôn, Quận 1, TP. Hồ Chí Minh', phone: '028 3882 0099', hours: '08:00 - 18:00', status: 'Open', region: 'Miền Nam' },
  { id: '3', name: 'Phòng giao dịch Đà Nẵng', type: 'Phòng giao dịch', address: '12 Nguyễn Văn Linh, Quận Hải Châu, Đà Nẵng', phone: '0236 3881 2233', hours: '08:00 - 17:00', status: 'Closed', region: 'Miền Trung' },
  { id: '4', name: 'ATM 24/7 - Vincom Bà Triệu', type: 'Cây ATM', address: 'Tầng 1, 191 Bà Triệu, Hai Bà Trưng, Hà Nội', phone: '1900 1122', hours: '24/7', status: 'Open', region: 'Miền Bắc' },
  { id: '5', name: 'Chi nhánh Cần Thơ', type: 'Chi nhánh/PGD', address: '45 Hòa Bình, Quận Ninh Kiều, Cần Thơ', phone: '0292 3776 1122', hours: '08:00 - 17:30', status: 'Open', region: 'Miền Nam' },
];
...
export default function Branches() {
  ...
  const filteredBranches = useMemo(() => {
    return initialBranches.filter(branch => {
      const matchSearch = branch.name.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          branch.address.toLowerCase().includes(searchTerm.toLowerCase());
      const matchType = activeType === 'Tất cả' || 
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h2 className="font-display font-bold text-24 text-primary tracking-tight">Tìm kiếm Chi nhánh / ATM</h2>
      </div>
        ...
        <div className="lg:col-span-4 space-y-4">
              ...
              {['Tất cả', 'Chi nhánh', 'ATM'].map((type) => (
                <button 
                  ...
                  className={cn(
                    ...
                    activeType === type ? "bg-primary text-white border-primary shadow-sm" : "bg-surface border-outline-variant text-on-surface-variant hover:bg-surface-container-low"
                  )}
                >
                  {type}
                </button>
          ...
          <div className="bg-white border border-outline-variant rounded-2xl shadow-sm overflow-hidden flex flex-col max-h-[600px]">
            <div className="p-4 border-b border-outline-variant bg-surface-container-low/30">
              <p className="text-[10px] font-bold text-outline uppercase tracking-widest">KẾT QUẢ TÌM KIẾM ({filteredBranches.length})</p>
            </div>
              ...
              {filteredBranches.map((branch) => (
                <div 
                  ...
                  className={cn(
                    ...
                    selectedBranch?.id === branch.id && "bg-primary/5 border-l-4 border-primary"
                  )}
                >
                  <div className="flex justify-between items-start mb-2">
                    <p className="text-sm font-bold text-primary group-hover:text-primary-container transition-colors leading-tight">{branch.name}</p>
                    <span className={cn(
                      "text-[9px] font-bold px-2 py-0.5 rounded-full uppercase ring-1 ring-inset",
                      branch.status === 'Open' ? 'bg-secondary/5 text-secondary ring-secondary/20' : 'bg-error/5 text-error ring-error/20'
                    )}>
                      {branch.status === 'Open' ? 'Mở cửa' : 'Đóng'}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-[10px] font-bold text-outline mb-4">
                    <div className={cn("w-2 h-2 rounded-full", branch.type.includes('Chi nhánh') ? 'bg-primary' : 'bg-tertiary-fixed')}></div>
        ...
        <div className="lg:col-span-8 space-y-6">
            ...
            {selectedBranch ? (
              <div className="z-10 flex flex-col items-center animate-in fade-in zoom-in duration-300">
                ...
                <div className="text-center px-6">
                  <h3 className="font-display font-bold text-2xl text-primary mb-2">{selectedBranch.name}</h3>
                  <p className="text-sm text-on-surface-variant max-w-[400px] mx-auto mb-6">{selectedBranch.address}</p>
              ...
              <div className="z-10 flex flex-col items-center gap-6 text-center px-6">
                ...
                <div>
                  <p className="font-display font-bold text-xl text-primary">Mạng lưới giao dịch toàn quốc</p>
                  <p className="text-sm text-on-surface-variant max-w-[320px] mt-2 italic">Vui lòng chọn một điểm giao dịch từ danh sách bên trái để xem thông tin chi tiết.</p>
                </div>
              </div>
            )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white border border-outline-variant p-6 rounded-2xl shadow-sm relative overflow-hidden group">
              ...
              <p className="text-xs text-on-surface-variant leading-relaxed mb-6">ATM thế hệ mới được trang bị máy nộp tiền tự động CRM, hỗ trợ nộp tiền mặt trực tiếp vào tài khoản 24/7 mà không cần qua quầy.</p>
              <div className="flex flex-wrap gap-2">
                {['Nộp tiền', 'Rút QR', 'Sinh trắc'].map(tag => (
                  <span key={tag} className="text-[9px] font-bold bg-surface-container text-primary px-2.5 py-1 rounded-full uppercase tracking-widest">{tag}</span>
                ))}
              </div>
            </div>
            <div className="bg-primary-container text-white p-6 rounded-2xl shadow-md relative overflow-hidden">
               <div className="relative z-10 h-full flex flex-col">
                  <h3 className="font-display font-bold text-sm mb-4 flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-secondary-fixed" /> Ưu tiên giao dịch (Priority)
                  </h3>
                  <p className="text-xs text-primary-fixed-dim leading-relaxed mb-6">Tận hưởng không gian giao dịch đẳng cấp và riêng tư tại các quầy chuyên biệt dành cho hội viên Priority.</p>
                  <button className="mt-auto text-xs font-bold text-secondary-fixed flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                    Tìm chi nhánh Priority gần nhất <ChevronRight className="w-4 h-4" />
                  </button>
               </div>
