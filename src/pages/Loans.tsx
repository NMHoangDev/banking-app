...
import { getLoans, createLoan, updateLoan } from "../services/loans.service";
import Modal from "../components/ui/Modal";

const initialLoans = [
  {
    id: "L-998271",
    customer: "Nguyễn Văn Lợi",
    amount: "2,500,000,000",
    interest: "8.5%",
    duration: "24",
    status: "Active",
    nextPayment: "12/11/2023",
    paid: "1,200,000,000",
  },
  {
    id: "L-998272",
    customer: "Công ty TechCorp",
    amount: "15,000,000,000",
    interest: "7.2%",
    duration: "60",
    status: "Active",
    nextPayment: "15/11/2023",
    paid: "3,500,000,000",
  },
  {
    id: "L-998273",
...
export default function Loans() {
  ...
  const handleOpenPayFlow = (loan: any) => {
    ...
    const remainingVal =
      parseInt(loan.amount.replace(/,/g, "")) -
      parseInt(loan.paid.replace(/,/g, ""));
  ...
  const executeLoanPayment = () => {
    ...
    const updated = loans.map((l) => {
      if (l.id === loanToPay.id) {
        ...
        const newStatus =
          newPaidVal >= totalLoan
            ? "Closed"
            : l.status === "Late"
              ? "Active"
              : l.status;
        return { ...l, paid: newPaidVal.toLocaleString(), status: newStatus };
      }
      return l;
    });
    setLoans(updated);
    ...
    // Persist change: update loan status (and optionally record payment elsewhere)
    const apiId = loanToPay.apiId;
    const newStatus = updated.find((l) => l.id === loanToPay.id)?.status;
    updateLoan(apiId, { status: newStatus })
      .then(() => {
        setPayStep(3);
      })
      .catch((err) => {
        ...
        alert(
          "Thanh toán không hoàn tất. Hệ thống sẽ khôi phục trạng thái trước đó.",
        );
        setLoans(prev);
        if (selectedLoan && selectedLoan.id === loanToPay.id)
  ...
  const handleCreateLoan = (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      interest_rate: parseFloat(newInterest) || 0,
      status: "PENDING",
    };
    ...
    createLoan(payload)
      .then((created: any) => {
        const mapped = {
          apiId: created.loan_id,
          id: `L-${created.loan_id}`,
          customer: newCustomer || created.customer_name || "Khách hàng",
          amount: (created.amount || payload.amount).toLocaleString(),
          interest: `${created.interest_rate ?? payload.interest_rate}%`,
          duration: `${newDuration}`,
          status: created.status === "PENDING" ? "Pending" : created.status,
          nextPayment: "-",
          paid: "0",
        };
      ...
      .catch((err) => {
        ...
        alert("Không thể tạo hồ sơ vay, thử lại sau.");
      });
  };
    ...
    const load = async () => {
      ...
      try {
        ...
        const mapped = rows.map((r: any) => ({
          ...
          interest: r.interest_rate ? `${r.interest_rate}%` : "0%",
          duration: r.duration_months ? `${r.duration_months}` : "0",
          status:
            r.status === "ACTIVE"
              ? "Active"
              : r.status === "LATE"
          ...
          nextPayment: r.next_payment
            ? new Date(r.next_payment).toLocaleDateString("vi-VN")
            : "-",
          paid: r.paid_amount ? r.paid_amount.toLocaleString() : "0",
        }));
        setLoans(mapped);
      } catch (err) {
        console.error("Failed to load loans", err);
        setLoansError(
          "Không thể tải danh sách khoản vay, hiển thị dữ liệu mẫu.",
        );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          {
            label: "TỔNG DƯ NỢ CẤP PHÁT",
            value:
              loans
                .filter((l) => l.status === "Active" || l.status === "Late")
                .reduce(
                  (sum, l) => sum + parseInt(l.amount.replace(/,/g, "")),
                  0,
                )
                .toLocaleString() + " VND",
          ...
          },
          {
            ...
            value: loans.filter((l) => l.status === "Late").length,
            ...
            <tbody>
                ...
                return (
                  <tr key={loan.id}>
                    ...
                    <td className="text-right">
                      <p className="font-numeric-data text-sm font-bold text-primary">
                        {loan.amount}
                      </p>
                      {loan.nextPayment !== "-" && (
                        <p className="text-[9px] text-outline font-bold uppercase tracking-tighter">
                          Hạn: {loan.nextPayment}
                        </p>
                      )}
                    </td>
                    <td className="text-center">
                      <p className="text-xs font-bold">
                        {loan.duration.includes("tháng")
                          ? loan.duration
                          : `${loan.duration} tháng`}
                      </p>
                      <p className="text-[9px] text-outline font-medium uppercase tracking-widest">
                        {loan.interest} APR
                      </p>
                    </td>
                    <td>
                      <div className="w-32 space-y-1">
                        ...
                        <p className="text-[9px] font-bold text-outline uppercase">
                          {percent}% đã trả ({loan.paid} VND)
                        </p>
                      </div>
                    </td>
                    <td className="text-center">
              ...
              {filteredLoans.length === 0 && (
                <tr>
                  ...
                  >
                    Không tìm thấy hồ sơ vay phù hợp
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <form onSubmit={handleCreateLoan} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              >
                <option value="Vay kinh doanh">
                  Vay phát triển kinh doanh
                </option>
      ...
      <Modal
        ...
        title="Thực hiện thanh toán nợ gốc / lãi"
      >
        {loanToPay && (
                ...
                <div className="p-4 bg-surface-container rounded-2xl border border-outline-variant/30">
                  ...
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-xs font-bold text-[#002147]">
                      {loanToPay.customer} ({loanToPay.id})
                    </span>
                    <span className="text-xs font-bold text-outline">
                      Dư nợ:{" "}
                      {(
                        parseInt(loanToPay.amount.replace(/,/g, "")) -
                        parseInt(loanToPay.paid.replace(/,/g, ""))
                      ).toLocaleString()}{" "}
                      VND
                    </span>
                  </div>
                  <div className="w-full h-1 bg-outline-variant rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.round((parseInt(loanToPay.paid.replace(/,/g, "")) / parseInt(loanToPay.amount.replace(/,/g, ""))) * 100)}%`,
                      }}
                    ></div>
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    <option value="1000000001">
                      1000000001 - Nguyễn Văn Thành • Số dư: 245,000,000 VND
                    </option>
                    <option value="1000000003">
                      1000000003 - Lê Minh Hoàng • Số dư: 18,500,000 VND
                    </option>
                  </select>
                </div>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-yellow-50/50 rounded-2xl border border-yellow-200 flex gap-3 items-start">
                  ...
                  <p className="text-xs text-amber-800 leading-relaxed">
                    Xác nhận thanh toán nợ. Sau khi ấn nút bên dưới, số dư tài
                    khoản của bạn sẽ lập tức bị trừ và số nợ sẽ được ghi giảm.
                  </p>
                </div>
                ...
                <div className="pt-4 flex justify-end gap-3 border-t border-outline-variant">
                  ...
                  <button
                    onClick={executeLoanPayment}
                    className="px-6 py-2 rounded-lg text-sm font-bold bg-[#002147] text-white shadow-sm hover:shadow-lg transition-all"
                  >
                    Xác nhận trả nợ
                  </button>
                </div>
              ...
              <div className="text-center py-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-on-surface-variant mt-1">
                    Hồ sơ dư nợ của khách hàng đã được cập nhật thành công.
                  </p>
                </div>
        ...
        {selectedLoan && (
          <div className="p-6 space-y-6">
            <div className="p-6 bg-surface-container-low rounded-3xl border border-outline-variant flex justify-between items-center shadow-inner">
              <div>
                <p className="text-[9px] font-bold uppercase text-outline mb-1">
                  Mã hồ sơ vay
                </p>
                <p className="font-numeric-data text-2xl text-primary font-bold">
                  {selectedLoan.id}
                </p>
              </div>
              <div
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              ...
              <div>
                ...
                <p className="text-sm font-bold">
                  {selectedLoan.duration.includes("tháng")
                    ? selectedLoan.duration
                    : `${selectedLoan.duration} tháng`}
                </p>
              </div>
              <div>
                <p className="text-[9px] font-bold uppercase text-outline mb-1">
              ...
              <div>
                ...
                <p className="text-sm font-bold text-red-600">
                  {(
                    parseInt(selectedLoan.amount.replace(/,/g, "")) -
                    parseInt(selectedLoan.paid.replace(/,/g, ""))
                  ).toLocaleString()}{" "}
                  VNĐ
                </p>
              </div>
            ...
            {(selectedLoan.status === "Active" ||
              ...
              <div className="border-t border-outline-variant pt-4">
                <h5 className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-3 flex items-center gap-2">
                  <History className="w-4 h-4" /> Lịch sử thanh toán định kỳ
                  (Repayments)
                </h5>
                <div className="space-y-2">
                  {[
                    {
                      period: "Kỳ 1 (Tháng 10/2023)",
                      amount: "50,000,000 VND",
                      date: "12/10/2023",
                      status: "PAID",
                    },
                    {
                      period: "Kỳ 2 (Tháng 11/2023)",
                      amount: "50,000,000 VND",
                      date: "12/11/2023",
                      status: "PAID",
                    },
                    {
                      period: "Kỳ 3 (Tháng 12/2023)",
                      amount: "50,000,000 VND",
                      date: "-",
                      status:
                        selectedLoan.status === "Late" ? "LATE" : "FUTURE",
                    },
                  ].map((rep, idx) => (
                    <div
                      key={idx}
                    ...
                    >
                      <div>
                        ...
                        <p className="text-[9px] text-outline">
                          Hạn trả: 12 hàng tháng • Thực trả: {rep.date}
                        </p>
                      </div>
                      <span
                        className={cn(
            ...
            <div className="pt-4 border-t border-outline-variant">
              <p className="text-[10px] font-bold uppercase text-outline mb-3">
                Tác vụ quản lý
              </p>
