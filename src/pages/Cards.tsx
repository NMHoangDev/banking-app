...
const initialCards = [
  { id: '4532 **** **** 8821', holder: 'Nguyễn Văn Thành', type: 'Credit Platinum', expiry: '12/28', limit: '200,000,000', used: '45,200,000', status: 'Active', brand: 'Visa', color: 'bg-primary-container', linkedAccount: '1000000001' },
  { id: '5421 **** **** 0092', holder: 'Lê Minh Hoàng', type: 'Debit Gold', expiry: '05/26', limit: '500,000,000', used: '12,000,000', status: 'Active', brand: 'Mastercard', color: 'bg-secondary-container', linkedAccount: '1000000003' },
  { id: '4921 **** **** 4432', holder: 'Trần Thị Phương', type: 'Credit Signature', expiry: '10/27', limit: '1,000,000,000', used: '850,000,000', status: 'Blocked', brand: 'Visa', color: 'bg-error-container', linkedAccount: '1000000021' },
  { id: '3567 **** **** 1120', holder: 'Vũ Anh Duy', type: 'Credit Classic', expiry: '03/25', limit: '50,000,000', used: '5,000,000', status: 'Active', brand: 'JCB', color: 'bg-tertiary-fixed', linkedAccount: '1000000002' },
];

const mockCardTxns: Record<string, any[]> = {
  '4532 **** **** 8821': [
    { merchant: 'Starbucks Coffee', date: 'Hôm nay, 08:30', amount: '125,000 VND', type: 'Dining' },
    { merchant: 'CGV Cinemas Cinema', date: 'Hôm qua, 20:15', amount: '220,000 VND', type: 'Entertainment' },
    { merchant: 'Grab Ride VietNam', date: '15/10/2023', amount: '45,000 VND', type: 'Transport' },
  ],
  '5421 **** **** 0092': [
    { merchant: 'Lotte Mart Supermarket', date: '14/10/2023', amount: '1,540,000 VND', type: 'Shopping' },
    { merchant: 'Petrolimex Gas', date: '12/10/2023', amount: '500,000 VND', type: 'Transport' },
  ],
  '4921 **** **** 4432': [
    { merchant: 'Apple Store Online', date: '08/10/2023', amount: '32,990,000 VND', type: 'Shopping' },
  ],
  '3567 **** **** 1120': [
    { merchant: 'Spotify Premium', date: '01/10/2023', amount: '59,000 VND', type: 'Entertainment' },
  ],
};
...
export default function Cards() {
  ...
  const executeToggleStatus = () => {
    ...
    setCards(prevCards => 
      prevCards.map(c => 
        c.id === cardToToggle.id 
          ? { ...c, status: c.status === 'Active' ? 'Blocked' : 'Active' }
          : c
      )
    );
    // If selected card in details is the one we updated, sync it
  ...
  const handleCreateCard = (e: React.FormEvent) => {
    ...
    const firstDigits = newCardBrand === 'Visa' ? '4532' : newCardBrand === 'Mastercard' ? '5421' : '3567';
    const generatedId = `${firstDigits} **** **** ${lastDigits}`;
    const colors = ['bg-primary-container', 'bg-secondary-container', 'bg-error-container', 'bg-tertiary-container'];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'TỔNG THẺ ĐANG CHẠY', value: cards.filter(c => c.status === 'Active').length, icon: <CreditCard className="w-5 h-5" />, color: 'primary' },
          { label: 'THẺ ĐÃ KHÓA', value: cards.filter(c => c.status === 'Blocked').length, icon: <Lock className="w-5 h-5" />, color: 'error' },
          { label: 'HẠN MỨC CẤP PHÁT', value: '1.75B', icon: <Sliders className="w-5 h-5" />, color: 'secondary' },
          { label: 'YÊU CẦU PHÁT HÀNH', value: '3', icon: <ShieldCheck className="w-5 h-5" />, color: 'tertiary' },
        ].map((stat, i) => (
            ...
            <tbody>
              {filteredCards.map((card, i) => (
                <tr key={card.id}>
                  ...
                  <td>
                    <p className="text-[10px] font-bold text-on-surface-variant uppercase">{card.type}</p>
                    <p className="text-[9px] text-outline font-mono font-bold">TK: {card.linkedAccount}</p>
                  </td>
                  <td>
                    ...
                    <div className="flex justify-end gap-1">
                      ...
                      <button 
                        ...
                        className={cn(
                          ...
                          card.status === 'Active' ? "hover:bg-red-50 text-red-600 hover:text-red-700" : "hover:bg-emerald-50 text-emerald-600 hover:text-emerald-700"
                        )}
                        title={card.status === 'Active' ? 'Khóa thẻ' : 'Mở khóa'}
                      >
      ...
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="Phát hành thẻ mới">
        <form onSubmit={handleCreateCard} className="p-6 space-y-4">
          <div className="space-y-1.5">
            ...
            >
              <option value="1000000001">1000000001 - Nguyễn Văn Thành ( saving )</option>
              <option value="1000000003">1000000003 - Lê Minh Hoàng ( payment )</option>
              <option value="1000000021">1000000021 - Trần Thị Phương ( locked )</option>
              <option value="1000000002">1000000002 - Vũ Anh Duy ( personal )</option>
            </select>
          </div>

          <div className="space-y-1.5">
            <label className="text-[10px] font-bold uppercase text-on-surface-variant">Họ & Tên chủ thẻ</label>
            <input 
              type="text"
              required
              value={newCardHolder}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold uppercase text-on-surface-variant">Hạng & Loại Thẻ</label>
              <select 
                value={newCardType}
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold uppercase text-on-surface-variant">Tổ chức thẻ (Brand)</label>
              <select 
                value={newCardBrand}
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold uppercase text-on-surface-variant">Hạn mức tín dụng cấp phát (VND)</label>
            <input 
              type="text" 
              required
              value={newCardLimit}
          ...
          <div className="pt-4 flex justify-end gap-3 border-t border-outline-variant">
            ...
            <button type="submit" className="px-6 py-2 rounded-lg text-sm font-bold bg-[#002147] text-white shadow-sm hover:shadow-lg transition-all active:scale-95">Xác nhận phát hành</button>
          </div>
        </form>
      </Modal>

      {/* Confirmation Lock/Unlock Modal */}
      <Modal isOpen={isConfirmModalOpen} onClose={() => setIsConfirmModalOpen(false)} title="Xác nhận thay đổi trạng thái">
        {cardToToggle && (
            ...
            <div className="flex gap-4 p-4 rounded-2xl bg-surface-container-low border border-outline-variant/30 items-start">
              ...
              <div>
                <p className="text-sm font-bold text-[#002147]">
                  Bạn chắc chắn muốn {cardToToggle.status === 'Active' ? 'KHÓA' : 'MỞ KHÓA'} thẻ này?
                </p>
                <p className="text-xs text-on-surface-variant mt-1.5">
                  Thẻ: <strong className="font-mono text-primary">{cardToToggle.id}</strong> <br />
                  Chủ thẻ: <strong>{cardToToggle.holder}</strong>
                </p>
                <p className="text-[11px] text-red-700 mt-2 italic">
                  * {cardToToggle.status === 'Active' ? 'Khi bị khóa, tất cả các giao dịch trực tuyến và rút tiền ATM qua thẻ này sẽ bị đình chỉ ngay lập tức.' : 'Sau khi mở khóa, chủ thẻ có thể giao dịch lại bình thường.'}
                </p>
              </div>
            </div>
            <div className="pt-4 flex justify-end gap-3 border-t border-outline-variant">
              <button onClick={() => { setIsConfirmModalOpen(false); setCardToToggle(null); }} className="px-6 py-2 rounded-lg text-sm font-bold text-on-surface-variant hover:bg-surface-container-low transition-all">Hủy bỏ</button>
              <button 
                onClick={executeToggleStatus} 
                className={cn(
                  "px-6 py-2 rounded-lg text-sm font-bold text-white shadow-sm hover:shadow-lg transition-all active:scale-95",
                  cardToToggle.status === 'Active' ? "bg-red-600 hover:bg-red-700" : "bg-emerald-600 hover:bg-emerald-700"
                )}
              >
                Đồng ý {cardToToggle.status === 'Active' ? 'Khóa thẻ' : 'Mở khóa'}
              </button>
            </div>
          </div>
        )}
      ...
      {/* Detail Modal */}
      <Modal isOpen={isDetailModalOpen} onClose={() => setIsDetailModalOpen(false)} title="Thông tin thẻ chi tiết" maxWidth="lg">
        {selectedCard && (
          <div className="p-6 space-y-6">
            <div className="flex justify-center p-6 bg-surface-container-low rounded-3xl border border-outline-variant relative overflow-hidden group">
              ...
              <div className={cn("w-80 h-48 rounded-2xl p-6 flex flex-col justify-between text-white shadow-2xl relative z-10 transition-transform hover:scale-[1.02] border border-white/10", selectedCard.color)}>
                <div className="flex justify-between items-start">
                  <div className="w-10 h-8 bg-yellow-400/80 rounded shadow-inner flex items-center justify-center font-bold text-primary/40 text-xs">CHIP</div>
                  <span className="font-display font-bold italic text-white/80 text-lg tracking-wider">{selectedCard.brand}</span>
                </div>
                <div>
                  <p className="font-numeric-data text-xl tracking-[0.2em] shadow-sm mb-4 text-center">{selectedCard.id}</p>
                  <div className="flex justify-between items-end">
                    <div>
                      <p className="text-[8px] uppercase opacity-60 mb-0.5 tracking-widest">Card Holder</p>
                      <p className="text-xs font-bold uppercase truncate max-w-[150px]">{selectedCard.holder}</p>
                    </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-3.5 bg-surface border border-outline-variant/30 rounded-2xl flex flex-col justify-between">
                <span className="text-[9px] font-bold text-outline uppercase tracking-wider">Hạn mức chi tiêu</span>
                <span className="text-sm font-bold text-primary mt-1">{selectedCard.limit} VND</span>
              </div>
              <div className="p-3.5 bg-surface border border-outline-variant/30 rounded-2xl flex flex-col justify-between">
                <span className="text-[9px] font-bold text-outline uppercase tracking-wider">Tài khoản liên kết</span>
                <span className="text-sm font-bold text-[#002147] mt-1 font-mono">{selectedCard.linkedAccount}</span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 rounded-xl bg-surface border border-outline-variant">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-outline">Bảo vệ thẻ khi chưa có nhu cầu sử dụng</p>
                  </div>
                </div>
                <button 
                  onClick={(e) => {
                  ...
                  className={cn(
                    ...
                    selectedCard.status === 'Active' ? "bg-red-50 text-red-700 hover:bg-red-100" : "bg-emerald-50 text-emerald-700 hover:bg-emerald-100"
                  )}
                >
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-surface border border-outline-variant">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold">Thanh toán trực tuyến (E-commerce)</p>
                    <p className="text-[9px] text-outline">Cho phép giao dịch trực tuyến qua Internet</p>
                  </div>
                </div>
            ...
            <div className="border-t border-outline-variant pt-4">
              <h5 className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider mb-3 flex items-center gap-2">
                <History className="w-4 h-4" /> Lịch sử giao dịch qua thẻ gần đây
              </h5>
              <div className="space-y-2">
                {(mockCardTxns[selectedCard.id] || []).map((txn, index) => (
                  <div key={index} className="flex justify-between items-center p-2.5 hover:bg-surface-container-low rounded-xl border border-outline-variant/20 transition-colors">
                    <div>
                      <p className="text-xs font-bold text-[#002147]">{txn.merchant}</p>
                      <p className="text-[9px] text-outline">{txn.date} • {txn.type}</p>
                    </div>
                    <span className="text-xs font-bold text-red-600">-{txn.amount}</span>
                  </div>
                ))}
                {(!mockCardTxns[selectedCard.id] || mockCardTxns[selectedCard.id].length === 0) && (
                  <p className="text-xs text-outline italic text-center py-4">Chưa có giao dịch nào qua thẻ này</p>
                )}
              </div>
            </div>
