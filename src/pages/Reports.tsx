...
const initialReports = [
  { id: 'REP-2023-001', name: 'Báo cáo Tài chính Q3/2023', type: 'Financial', date: '30/09/2023', size: '2.4 MB', creator: 'Nguyễn Văn Thành', status: 'Completed' },
  { id: 'REP-2023-002', name: 'Báo cáo Doanh thu Thu hộ & Dịch vụ Tháng 10', type: 'Revenue', date: '31/10/2023', size: '1.8 MB', creator: 'Lê Minh Hoàng', status: 'Completed' },
  { id: 'REP-2023-003', name: 'Phân tích Rủi ro & Tín dụng Quá hạn', type: 'Risk', date: '12/11/2023', size: '4.2 MB', creator: 'Trần Minh Hoàng', status: 'Pending' },
  { id: 'REP-2023-004', name: 'Nhật ký Kiểm toán & Bảo mật định kỳ', type: 'Audit', date: '15/11/2023', size: '3.1 MB', creator: 'Lê Văn Thành', status: 'Completed' },
];
...
export default function Reports() {
  ...
  const handleCreateReport = (e: React.FormEvent) => {
    ...
    const newRep = {
      ...
      size: `${(1.0 + Math.random() * 4).toFixed(1)} MB`,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <h2 className="font-display font-bold text-24 text-primary tracking-tight">Thống kê dữ liệu & Báo cáo tài chính</h2>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'TỔNG TÀI SẢN HỆ THỐNG', value: '45,280,000,000,000 VND', icon: <TrendingUp className="w-5 h-5" />, color: 'primary', trend: '+12.4% so với năm ngoái' },
          { label: 'DOANH THU THUẦN Q3', value: '184,500,000,000 VND', icon: <DollarSign className="w-5 h-5" />, color: 'secondary', trend: '+8.2% so với Q2' },
          { label: 'CHI PHÍ VẬN HÀNH', value: '42,100,000,000 VND', icon: <TrendingDown className="w-5 h-5" />, color: 'error', trend: '-2.5% tối ưu chi phí' },
          { label: 'HẠN MỨC TÍN DỤNG CẤP', value: '18,500,000,000,000 VND', icon: <BarChart3 className="w-5 h-5" />, color: 'tertiary', trend: 'Tỷ lệ nợ xấu 0.45%' },
        ].map((stat, i) => (
          <div key={i} className="bg-white p-5 rounded-2xl border border-outline-variant shadow-sm flex flex-col justify-between">
            <div>
              ...
              <p className="label-uppercase text-on-surface-variant text-[10px] mb-1">{stat.label}</p>
              <p className="font-display font-bold text-lg text-primary leading-tight">{stat.value}</p>
            </div>
            <p className="text-[10px] text-emerald-600 font-bold mt-4">{stat.trend}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-outline-variant rounded-3xl p-6 shadow-sm lg:col-span-2 flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-center mb-6">
              <div>
                <h4 className="font-display font-bold text-base text-primary">Biểu đồ phân tích chi tiêu & nguồn vốn</h4>
                <p className="text-xs text-on-surface-variant">Phân bổ nguồn tiền và hạn mức thanh khoản trong 6 tháng qua</p>
              </div>
              <div className="flex items-center gap-1.5 p-1 bg-surface-container rounded-xl border border-outline-variant/30 text-[10px] font-bold">
                <button className="px-2.5 py-1 bg-white rounded-lg shadow-sm text-primary">Nguồn Vốn</button>
                <button className="px-2.5 py-1 hover:bg-white/50 rounded-lg text-outline">Chi Tiêu</button>
              </div>
            </div>
            ...
            <div className="h-64 flex items-end justify-between gap-4 pt-4 border-b border-outline-variant/30">
              {[
                ...
                <div 
                  ...
                  onMouseEnter={() => setSelectedChartSegment(`${bar.month}: Nguồn vốn ${bar.capital}B, Chi tiêu ${bar.spending}B`)}
                ...
                >
                  ...
                  <span className="text-[10px] font-bold text-on-surface-variant">{bar.month}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-4 p-3 bg-surface-container rounded-2xl border border-outline-variant/30 flex justify-between items-center min-h-[48px]">
            <span className="text-xs font-medium text-on-surface-variant flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-[#002147]"></span> Nguồn vốn hệ thống
              <span className="w-2.5 h-2.5 rounded-full bg-orange-400 ml-2"></span> Chi tiêu vận hành
            </span>
        ...
        <div className="bg-white border border-outline-variant rounded-3xl p-6 shadow-sm flex flex-col justify-between">
          <div>
            <h4 className="font-display font-bold text-base text-primary mb-1">Cơ cấu danh mục đầu tư</h4>
            <p className="text-xs text-on-surface-variant mb-6">Tỷ trọng tài sản phát sinh thu nhập</p>

            <div className="space-y-4">
              {[
                { name: 'Tín dụng doanh nghiệp', percent: 45, val: '20.3T VND', color: 'bg-[#002147]' },
                { name: 'Tín dụng tiêu dùng cá nhân', percent: 30, val: '13.5T VND', color: 'bg-emerald-600' },
                { name: 'Thanh khoản ngân hàng', percent: 15, val: '6.7T VND', color: 'bg-amber-500' },
                { name: 'Khác', percent: 10, val: '4.5T VND', color: 'bg-slate-400' },
              ].map((item, idx) => (
                <div key={idx} className="space-y-1">
                  <div className="flex justify-between text-xs font-bold">
                    ...
                    <span className="text-primary">{item.percent}% ({item.val})</span>
                  </div>
          ...
          <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-200 mt-6 flex gap-2 items-start">
            ...
            <p className="text-[10px] text-emerald-800 leading-normal">
              Hạn mức thanh khoản của toàn hệ thống được duy trì ở mức cực kỳ an toàn <strong>(15%)</strong> theo tiêu chuẩn Basel III.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-outline-variant rounded-2xl shadow-sm overflow-hidden flex flex-col">
        <div className="p-4 flex flex-wrap items-center justify-between gap-4 border-b border-outline-variant bg-surface-container-low/30">
          <h4 className="font-display font-bold text-base text-primary">Kho lưu trữ báo cáo định kỳ</h4>
                ...
                <tr key={rep.id}>
                  ...
                  <td className="text-center">
                    <span className={cn(
                      ...
                      rep.status === 'Completed' ? 'bg-secondary-container text-on-secondary-container' : 'bg-surface-container-highest text-on-surface-variant animate-pulse'
                    )}>
      ...
      <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} title="Yêu cầu tạo báo cáo mới">
        <form onSubmit={handleCreateReport} className="p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold uppercase text-on-surface-variant">Khoảng thời gian phân tích</label>
            <select 
              value={newRange}
          ...
          <div className="pt-4 flex justify-end gap-3 border-t border-outline-variant">
            <button type="button" onClick={() => setIsCreateModalOpen(false)} className="px-6 py-2 rounded-lg text-sm font-bold text-on-surface-variant hover:bg-surface-container-low transition-all">Hủy bỏ</button>
            <button type="submit" className="px-6 py-2 rounded-lg text-sm font-bold bg-[#002147] text-white shadow-sm hover:shadow-lg transition-all active:scale-95">Tạo báo cáo</button>
          </div>
        </form>
      </Modal>

      {/* View Detail Report Modal */}
      <Modal isOpen={isDetailModalOpen} onClose={() => setIsDetailModalOpen(false)} title="Bản xem trước báo cáo điện tử">
        {selectedReport && (
          <div className="p-6 space-y-6">
            <div className="p-6 bg-surface-container rounded-3xl border border-outline-variant flex flex-col items-center text-center">
              ...
              <h4 className="font-display font-bold text-base text-primary">{selectedReport.name}</h4>
              <p className="text-[9px] font-bold uppercase tracking-widest text-outline">{selectedReport.id}</p>
            </div>

            <div className="space-y-3 text-xs">
              <div className="flex justify-between items-center py-2 border-b border-outline-variant/30">
                <span className="text-on-surface-variant">Phân loại báo cáo</span>
                <span className="font-bold text-primary uppercase">{selectedReport.type}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-outline-variant/30">
              ...
              <div className="flex justify-between items-center py-2 border-b border-outline-variant/30">
                <span className="text-on-surface-variant">Chịu trách nhiệm lập</span>
            ...
            <div className="pt-4 border-t border-outline-variant space-y-3">
              <p className="text-[10px] font-bold uppercase text-outline">Nội dung tóm tắt & Kế hoạch</p>
              <div className="p-4 bg-surface-container rounded-2xl border border-outline-variant/30 leading-relaxed text-xs text-on-surface-variant">
                Báo cáo ghi nhận hiệu suất tăng trưởng tài sản lõi của tổ chức tăng 12.4% so với cùng kỳ năm trước. Dòng tiền gửi (deposits) của khách hàng tiếp tục là động lực thanh khoản chính. Tỷ lệ nợ xấu ở mức cực thấp 0.45%.
              </div>
            </div>
